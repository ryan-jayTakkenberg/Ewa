...
it('has create button', () => {
    ...
    expect(button?.exists(), `missing create user button`).toBe(true);
    expect(button.element.disabled, `Create user button is disabled`).toBeFalsy();
});
...
it('has table', () => {
    ...
    expect(userTableHeader?.exists(), `missing a user table`).toBe(true);
});
