...
const MODAL_TITLE_CONTAINS = 'Create';
const PRODUCT_BUTTON_CONTAINS = 'Create';

beforeEach(() => wrapper = mount(CreateProductModal));
...
it('has create button', () => {
    // find the create button to click
    ...
    expect(confirmButton?.exists(),
        `missing create product button`)
        .toBe(true);

    expect(confirmButton.element.disabled,
        `Create product button should be disabled until all input fields are filled in`)
        .toBeTruthy();
});
