...
export default {
  ...
  props: {
    project: {
      type: Project,  // Use the UserModel as the prop type
      required: true,
    },
    isChecked: {
      type: Boolean,
  ...
  methods: {
    editProject() {
      this.$emit('click-edit-user', this.project);  // Emit the userModel directly
    },
    toggleCheckbox() {
  ...
  <tr class="table-row">
    ...
    <th scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap ">
      <div class="pl-3">
        ...
        <div class="font-normal text-gray-500">{{ project.installTeamId }}</div>
      </div>
    </th>
    <td class="px-6 py-4">{{ project.clientName }}</td>
    <td class="px-6 py-4">
      <div class="flex items-center">{{ project.installDate }}</div>
    </td>

    <!-- Edit User -->
    <td class="px-6 py-4">
      <div @click="editProject" class="edit-user-btn">Edit project</div>
    </td>
